(function () {
    ...
    /* ─── PROXY TEST — real TCP reachability check ─── */
    function testProxyReachable(host, port, type, callback) {
        fetch('/api/proxy/check', {
            ...
            .then(function (d) { callback(d.alive, d.alive ? 'Online' : (d.reason || 'Refused')); })
            .catch(function (err) {
                /* Backend truly offline — show real error, do NOT simulate success */
                callback(false, 'Backend unreachable: ' + (err.message || 'network error'));
            });
    }
    ...
    function startTrafficSim() {
        ...
        /* Real heartbeat: test proxy is still reachable every 15s */
        trafficT = setInterval(function () {
            if (!connected) return;
            testProxyReachable(sel.host, parseInt(sel.port), sel.type, function (alive) {
                if (alive) {
                    log('[KEEP] Heartbeat OK → ' + sel.host + ':' + sel.port + ' (' + sel.ping + 'ms)', 'info');
                } else {
                    log('[WARN] Node heartbeat failed — connection may be unstable.', 'warning');
                }
            });
        }, 15000);
    }
    ...
    function startStats() {
        ...
        /* Show server-reported ping; data counters are not available without a real tunnel agent */
        if (sSent) sSent.textContent = '—';
        if (sRecv) sRecv.textContent = '—';
        if (sPing) sPing.textContent = sel.ping ? sel.ping + ' ms' : '— ms';

        if (statsT) clearInterval(statsT);
        /* Real uptime only — no fake data counters */
    ...
    /* ── KILL SWITCH — Manual trigger + real breach detection ── */
    function triggerKillSwitch(reason) {
        if (!connected && !connecting) return;
        log('[KILL SWITCH] \u26a0 ACTIVATED — ' + (reason || 'Manual trigger'), 'error');
        disconnect();
        killSwitchActive = true;
        protBadges.forEach(function (el) {
            el.className = 'badge danger';
            el.textContent = 'KILL SWITCH ACTIVE';
        });
        if (btnLabel) {
            btnLabel.textContent = 'KILL SWITCH';
            btnLabel.classList.add('kill');
        }
        setTimeout(function () {
            killSwitchActive = false;
            if (btnLabel && btnLabel.textContent === 'KILL SWITCH') {
                btnLabel.textContent = 'REPROX';
                btnLabel.classList.remove('kill');
            }
            protBadges.forEach(function (el) {
                if (el.className.indexOf('KILL') !== -1) {
                    el.className = 'badge danger';
    ...
    function handleBreach(e) {
        e.preventDefault();
        if (!connected) {
            log('[KILL SWITCH] Must be connected to activate.', 'warning');
            openLog();
            return;
        }
        openLog();
        log('[KILL SWITCH] \u25ba Running real IP breach check...', 'info');
        /* Real breach check: compare current IP to the one captured at connect-time */
        fetch('/api/ip', { signal: AbortSignal.timeout ? AbortSignal.timeout(5000) : undefined })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                var currentIp = data.ip || '';
                log('[KILL SWITCH] Fetched IP: ' + currentIp, 'system');
                if (realIpAddress && realIpAddress !== '\u2014' && currentIp && currentIp === realIpAddress) {
                    /* Real IP exposed — fire kill switch */
                    log('[KILL SWITCH] \u2717 IP BREACH DETECTED! Real IP exposed. Disconnecting...', 'error');
                    triggerKillSwitch('IP leak detected — real IP visible to exit node');
                } else if (!currentIp) {
                    log('[KILL SWITCH] \u26a0 Could not verify IP — connection integrity unknown.', 'warning');
                } else {
                    log('[KILL SWITCH] \u2713 IP check PASSED — proxy IP active, no leak detected.', 'success');
                }
            })
            .catch(function () {
                log('[KILL SWITCH] \u26a0 IP check failed — backend unreachable.', 'warning');
            });
    }
